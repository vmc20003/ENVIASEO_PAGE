...
import './AlumbradoTableSimple.css';

const AlumbradoTableSimple = ({ 
  data = [], 
  ...
  const handleLoadData = useCallback(async () => {
    setLoading(true);
    try {
      console.log('🔄 Cargando datos de archivos existentes...');
      const response = await fetch('http://localhost:5000/all-records?getAll=true');
  ...
  const handleUploadFile = useCallback(async (fileName) => {
    setLoading(true);
    try {
      console.log('📤 Procesando archivo individual:', fileName);
      
      // En lugar de enviar el archivo, vamos a procesar los datos del archivo existente
      const response = await fetch(`http://localhost:5000/records-by-file/${encodeURIComponent(fileName)}`);
  ...
  // Procesar datos de alumbrado
  const processAlumbradoData = useCallback((rawData) => {
  ...
  // Datos procesados
  const processedData = useMemo(() => processAlumbradoData(data), [data, processAlumbradoData]);
  ...
  const handleOpenExplorer = useCallback(async () => {
    const folderPath = 'C:\\Users\\Usuario\\Documents\\PAGINAS\\ENVIASEO_PAGE_ORIGINAL\\backend\\uploads_excel';
  ...
  return (
    <div className="alumbrado-table-simple">
...
export default AlumbradoTableSimple;
