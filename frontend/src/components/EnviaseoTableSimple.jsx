...
import './EnviaseoTableSimple.css';

const EnviaseoTableSimple = ({ 
  data = [], 
  ...
  const handleLoadData = useCallback(async () => {
    setLoading(true);
    try {
      console.log('🔄 [Enviaseo] Cargando datos de archivos existentes...');
      const response = await fetch('http://localhost:5001/all-records?getAll=true');
      
      if (response.ok) {
        const result = await response.json();
        console.log('✅ [Enviaseo] Datos cargados:', result);
      ...
      console.error('❌ [Enviaseo] Error cargando datos:', error);
  ...
  const handleUploadFile = useCallback(async (fileName) => {
    setLoading(true);
    try {
      console.log('📤 [Enviaseo] Procesando archivo individual:', fileName);
      
      // En lugar de enviar el archivo, vamos a procesar los datos del archivo existente
      const response = await fetch(`http://localhost:5001/records-by-file/${encodeURIComponent(fileName)}`);
      
      if (response.ok) {
        const result = await response.json();
        console.log('✅ [Enviaseo] Archivo procesado correctamente:', result);
      ...
      console.error('❌ [Enviaseo] Error procesando archivo:', error);
  ...
  // Procesar datos de enviaseo
  const processEnviaseoData = useCallback((rawData) => {
  ...
  // Datos procesados
  const processedData = useMemo(() => processEnviaseoData(data), [data, processEnviaseoData]);
  ...
  const handleOpenExplorer = useCallback(async () => {
    const folderPath = 'C:\\Users\\Usuario\\Documents\\PAGINAS\\ENVIASEO_PAGE_ORIGINAL\\backend-enviaseo-control-acceso\\uploads_excel';
  ...
  return (
    <div className="enviaseo-table-simple">
...
export default EnviaseoTableSimple;